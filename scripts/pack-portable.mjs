...
【数据存在哪】
图标项目数据保存在浏览器本地（IndexedDB）里，本软件本身不含任何项目数据。
浏览器对本地文件页面（file://）不按文件夹区分存储，实测行为如下：
- 移动 / 复制本文件夹不会丢数据；
- 同一台电脑、同一个浏览器里，任意位置的本地 SnFont 页面看到的是同一份数据
  （因此不能用"多解压几份"来隔离不同项目）；
- 换电脑、换浏览器、清理浏览器数据之前，请用「导出 ▾ → 下载项目」保存 Project 备份，
  在新环境用「导入 ▾ → 导入 SVG」选择该 project.json 即可完整恢复；
- 想清空数据：在应用里删除图标，或清除该浏览器的站点数据。
