...
- 导入 SVG / 图片转 SVG 的预览均按项目设置的 SVG 尺寸渲染（点击下方卡片切换大预览，与设置抽屉「SVG 尺寸」统一）；图片转 SVG 结果可逐张下载 SVG，供 Illustrator 等矢量工具精修后再导入
...
1. **PWA 支持**：manifest.json（名称/图标 192+512/主题色）+ Service Worker 离线缓存 + install 提示。
   - 技术独立于现有功能（PWA=部署壳，功能=功能层），互不阻塞；后加的依赖（如 esm-potrace-wasm 单文件内联 wasm）只需重新 build 即自动纳入缓存。
   - 前提：不破坏现有离线双击运行（server.mjs + dist）；localhost 下零联网，公网分享才需 HTTPS 托管；dist 约 1.2MB 原始 / gzip ≈430KB，缓存后离线秒开。
2. **图片透视纠正**（暂缓评估中）：手机拍摄的照片斜拍/透视变形，先做"四点点选 → 单应性变换拉平 → 再矢量化"的独立能力；当前 potrace 仅适合白底/透明底截图类源图。
3. **噪点增强过滤**（暂缓）：对照片四周的大面积杂物/边缘线框，考虑增加"仅保留最大连通域"等更强过滤；当前已有按面积的去噪点滑杆（turdsize 0~20）。
