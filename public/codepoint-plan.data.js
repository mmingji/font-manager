...
window.__SNFONT_CODEPOINT_PLAN__ = {
  "_readme": [
    "本文件是本项目的「码位规划配置」，可直接用记事本编辑，保存后刷新页面即生效。",
    "【project_alloc｜本项目保留区】最顶部：新增/导入图标从这里按顺序取用码位；设置面板显示本项目在该范围内的占用情况。",
    "【reference_sections｜参考字体占用分段】定义参考图标集（unicode-map.data.js）的码位统计口径；各段边界可改。",
    "  段内的占用数字由应用按最新映射实时统计，此处的 snapshotOccupied 只是生成时的参考值（不作为显示来源）。",
    "【pua_ranges｜PUA 段说明】文档性质，供码位规划参考。"
  ],
  "version": 2,
  "project_alloc": {
    "range": "EE00–EFFF",
  ...
  "reference_sections": {
    "_note": "各段边界（start/end）可编辑；占用数字由应用按 unicode-map.data.js 实时统计",
    "bmp": {
      "start": "U+E000",
      "end": "U+F8FF"
    },
    "sections": [
      ...
      },
      {
        ...
        "start": "U+F000",
        "end": "U+F0FF",
        "snapshotOccupied": 202
      },
      {
        "range": "F100–F8FF",
        "start": "U+F100",
        "end": "U+F8FF",
        "snapshotOccupied": 1927
      }
    ]
  },
  "pua_ranges": {
